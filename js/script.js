...
$(document).ready(function() {
    ...
    owl.on('changed.owl.carousel', function(event) {
        setTimeout(function() {
            ...
            $(".photo-title").addClass('changing').html(activeEls.find(".item").data('title'));
            setTimeout(function() {
                $(".photo-title").removeClass('changing');
            }, 200);
        }, 100);
    });

    var images = $(".album-container .item img");
    images.each(async function() {
        var imgUrl = $(this).data('img-src');
        $(this).attr('src', imgUrl);
    });

    $('.album-container').imagesLoaded().progress(function(instance, image) {
